...
this.attachOnClick = function(functionName) {
this.eventOnClick = functionName;}
...
if (this._disabled) return;var newPos   = this._getNewLocationFromCursor(ctrl);var valByPos = this._getValueByPosition(newPos);var val      = this._roundToGrid(valByPos, ctrl.knobId);var valInternal = (ctrl.knobId == 1) ? this._valueInternal : this._valueInternal2;if (val != valInternal) {
newPos = this._getPositionByValue(val, ctrl.knobId);if (ctrl.knobId == 1) {
...
var knobId = 1;}
val = this._roundToGrid(val, knobId);if (val != this._valueInternal) {
newPos = this._getPositionByValue(val, knobId);if (knobId == 1) {
this._valueInternal = val;} else {
this._valueInternal2 = val;}
...
val = this['minVal'+k];}
val = this._roundToGrid(val, knobId);var newPos = this._getPositionByValue(val, knobId);if (val != this['_valueInternal'+k]) {
...
if (typeof(knobId) == 'undefined') knobId = 1;val = this._roundToGrid(val, knobId);var newPos = this._getPositionByValue(val, knobId);var valInternal = (knobId == 1) ? this._valueInternal : this._valueInternal2;if (val != valInternal) {
if (knobId == 1) {
this._valueInternal = val;} else {
this._valueInternal2 = val;}
...
var pos = this['ctrl'+k].minX + ((this['ctrl'+k].maxX - this['ctrl'+k].minX) * myPercent);if (pos < this['ctrl'+k].minX) {
pos = this['ctrl'+k].minX;} else if (pos > this['ctrl'+k].maxX) {
pos = this['ctrl'+k].maxX;}
} else {
var pos = this['ctrl'+k].minY + ((this['ctrl'+k].maxY - this['ctrl'+k].minY) * myPercent);if (pos < this['ctrl'+k].minY) {
